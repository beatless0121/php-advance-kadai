...
if (isset($_POST['submit'])) {
 ...
 if (isset($_GET['id'])) {
     try {
         ...
         // booksテーブルからgenre_codeカラムのデータを取得するためのSQL文を変数$sql_select_genre_codesに代入する
         ...
         <article class="registration">
             ...
             <form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
                 <div>
                     ...
                     <input type="number" id="book_code" name="book_code" value="<?= $books['book_code'] ?>" min="0" max="100000000" required>
 
                     <label for="book_name">書籍名</label>
                     <input type="text" id="book_name" name="book_name" value="<?= $books['book_name'] ?>" maxlength="50" required>
 
                     <label for="price">単価</label>
                     <input type="number" id="price" name="price" value="<?= $books['price'] ?>" min="0" max="100000000" required>
 
                     <label for="stock_quantity">在庫数</label>
                     <input type="number" id="stock_quantity" name="stock_quantity" value="<?= $books['stock_quantity'] ?>" min="0" max="100000000" required>
